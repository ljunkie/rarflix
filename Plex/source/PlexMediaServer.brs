...
Function librarySectionContent(key) As Object
    ...
    for each videoItem in xml.Video
    	...
    	'* This results in dog slow navigation if I use the full thumbnail image. If I remember
    	'* correctly Elan mentioned image transcoding from PMS. Why they don't cache
    	'* the images rather than hitting the server I don't know. Maybe I need to cache myself
    	'* For arced-portrait mode
    	'* HD: 214x306
    	'* SD: 158x204
    	'video.SDPosterURL = m.serverUrl + videoItem@thumb
    	'video.HDPosterURL = m.serverUrl + videoItem@thumb
    	video.Key = videoItem@key
    	'* TODO: deal with multiple parts
